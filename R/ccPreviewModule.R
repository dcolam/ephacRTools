...
#'
#' Interactive AP detection preview embedded as a reusable Shiny module.
#' Can be embedded in any Shiny app (e.g. tinySEV).
#'
#' @param id Module namespace ID.
#' @name ccPreview
NULL

# ── Internal helpers (no namespacing needed) ──────────────────────────────────
      ...
      column(3,
        wellPanel(
          ...
          tags$div(style = "display:flex; gap:4px;",
            shinyFiles::shinyFilesButton(ns("add_files2"), "Add files",
              title = "Select raw trace Parquets", multiple = TRUE,
              buttonType = "default btn-sm"),
            shinyFiles::shinyDirButton(ns("add_folder2"), "Add folder",
              title = "Add all .parquets in folder",
              buttonType = "default btn-sm")
          ),
          ...
          actionButton(ns("scan_btn"), "Scan wells", icon = icon("search"),
                       class = "btn-primary btn-sm btn-block"),
          tags$hr(),

          uiOutput(ns("plate_selector_ui")),
          tags$div(style = "position:relative;",
            plotOutput(ns("plate_grid"), height = "180px",
          ...
          fluidRow(
            column(6, checkboxInput(ns("quick_mode"),    "This sweep only", TRUE)),
            column(6, checkboxInput(ns("show_all_sweeps"), "Overlay all",   FALSE))
          ),
          tags$hr(),
          ...
          bslib::accordion(
            ...
            bslib::accordion_panel(
              "Detection Config",
              sliderInput(ns("dvdt_thr"),      "dV/dt threshold (mV/ms)", 5,  100, 25,  step = 1),
              sliderInput(ns("smooth_ms"),     "Smoothing (ms)",           0,    2,  0.2, step = 0.05),
              sliderInput(ns("refractory_ms"), "Refractory (ms)",        0.5,   10,  2,   step = 0.5),
              sliderInput(ns("min_peak_v"),    "Min peak voltage (mV)",  -60,   20, -20,  step = 5),
              fluidRow(
                column(6, numericInput(ns("min_height"), "Min height (mV)", NA, step = 1)),
                column(6, numericInput(ns("prominence"), "Prominence (mV)", NA, step = 1))
              ),
              helpText("Leave height / prominence blank for adaptive threshold."),
              tags$hr(style = "margin:6px 0;"),
              fluidRow(
                column(6, downloadButton(ns("export_cfg"), "Export",
                                         icon = icon("download"),
                                         class = "btn-sm btn-outline-secondary btn-block")),
                column(6, actionButton(ns("reset_cfg"), "Defaults",
                                       icon = icon("undo"),
                                       class = "btn-sm btn-outline-secondary btn-block"))
              ),
          ...
          actionButton(ns("run_btn"), "Run on all loaded parquets",
                       icon = icon("play"), class = "btn-success btn-sm btn-block")
        )  # /wellPanel
      ),  # /column 3
          ...
          column(8,
            div(class = "panel panel-default",
              ...
              div(class = "panel-body", style = "padding:6px;",
                  plotOutput(ns("trace_plot"), height = "480px")),
              div(class = "panel-footer", textOutput(ns("ap_count_msg")))
            )
          ),
          column(4,
            tabsetPanel(type = "tabs",
              ...
              tabPanel("Phase Plane",
                plotOutput(ns("phase_plot"), height = "420px"),
                helpText("dV/dt vs V.  ▲ = dV/dt max,  ▼ = dV/dt min per AP.")
              )
            )
          )
        )
...
ccPreviewServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    shinyFiles::shinyFileChoose(input, "add_files2",  roots = .cc_roots, filetypes = "parquet")
    shinyFiles::shinyDirChoose(input,  "add_folder",  roots = .cc_roots)
    shinyFiles::shinyDirChoose(input,  "add_folder2", roots = .cc_roots)
    ...
    # ── File management ──────────────────────────────────────────────────────
    .add_parquets <- function(input_val) {
      req(is.list(input_val))
      new <- shinyFiles::parseFilePaths(.cc_roots, input_val)$datapath
      parquet_paths(unique(c(parquet_paths(), new[file.exists(new)])))
    }
    .add_folder <- function(input_val) {
      req(is.list(input_val))
      folder <- shinyFiles::parseDirPath(.cc_roots, input_val)
      req(nzchar(folder))
    ...
    observeEvent(input$add_files,   .add_parquets(input$add_files))
    observeEvent(input$add_files2,  .add_parquets(input$add_files2))
    observeEvent(input$add_folder,  .add_folder(input$add_folder))
    observeEvent(input$add_folder2, .add_folder(input$add_folder2))
    ...
    observeEvent(input$scan_btn, {
      ...
      withProgress(message = "Scanning wells…", {
        ...
        if (!is.null(result)) {
          ...
          selected_well_key(.cc_make_well_key(best$well_id, best$plate_id, best$parquet_path))
        }
        scan_results(result)
      })
    })
    ...
    output$plate_grid <- renderPlot({
      ...
      p <- ggplot2::ggplot(df, ggplot2::aes(col_num, row_idx, fill = n_ap_sweeps)) +
        ...
        ggplot2::scale_y_reverse(breaks = seq_len(n_rows), labels = LETTERS[seq_len(n_rows)]) +
    ...
    cfg_inputs <- reactive({
      list(dvdt_thr = input$dvdt_thr, smooth_ms = input$smooth_ms,
           ...
           min_height = input$min_height, prominence = input$prominence,
           quick = input$quick_mode, sweep = input$selected_sweep)
    }) |> debounce(500)
    ...
    ap_result <- reactive({
      ...
      tryCatch(
        previewAPDetection(
          parquet_path = info$path, well_id = info$well_id, plate_id = info$plate_id,
          sweeps = sweeps_arg,
          cfg = cc_config(
            v_is_volts = isTRUE(input$v_is_volts),
    ...
    output$trace_plot <- renderPlot({
      ...
      if (show_all) {
        p <- ggplot2::ggplot(plot_df,
               ggplot2::aes(t_s, v_mV, colour = as.numeric(sweep), group = factor(sweep))) +
    ...
    output$ap_count_msg <- renderText({
      ap <- ap_result(); req(ap)
      sw <- input$selected_sweep; total <- nrow(ap$aps)
    ...
    output$sweep_table <- renderTable({
      ...
      out
    }, digits = 1, striped = TRUE, hover = TRUE, spacing = "s", width = "100%", align = "r")
    ...
    observeEvent(input$run_btn, {
      ...
      showNotification(paste0("Done! Results in: ", out_dir), type = "message", duration = 10)
    })
